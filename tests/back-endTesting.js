...
test('Testing the router for the static files', (t) => {
  shot.inject(router, { method: 'get', url: '/style.css' }, (res) => {
    t.equal(
      res.statusCode,
      200,
      'should respond with status code of 200 for the css file',
    );
  ...
  shot.inject(router, { method: 'get', url: '/main.js' }, (res) => {
    t.equal(
      res.statusCode,
      200,
      'should respond with status code of 200 for the JS file',
    );
...
test('Testing the router for the time api', (t) => {
  shot.inject(
    router,
    { method: 'get', url: '/submit?search=Amsterdam' },
    (res) => {
      t.equal(
        res.statusCode,
        200,
        'should respond with status code of 200 for the JSON data',
      );
      t.notEqual(
        res.payload,
        undefined,
        'response should contain a JSON object',
      );
    },
  );
  t.end();
